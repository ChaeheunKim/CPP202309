...
User my_user; //클래스 정의
bool CheckUser(User user); //체력 체크 함수
...
int main() {
	...
	while (1) { // 사용자에게 계속 입력받기 위해 무한 루프
		...
		//체력이 0이 되었는 지 체크
		bool hp_check = CheckUser(my_user);
		if (hp_check == false) {
			cout << "실패" << endl;
...
void checkState(int map[][mapX], int user_x, int user_y) {
			int posState = map[user_y][user_x];
				...
				case 2:
					...
					cout << "체력은 " << my_user.GetHP() <<"입니다."<<  endl;
					break;
				case 3:
					cout << " 포션이 있습니다. 체력이 2만큼 회복됩니다."<<endl;
					my_user.increaseHP(2); //체력 2 회복
					cout << "체력은 " << my_user.GetHP() << "입니다."<< endl;
					break;
...
bool CheckUser(User user) {
	if (user.GetHP() <= 0)
		return false; //체력이 0 이하면 false 반환
	else
